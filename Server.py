...
def preprocess():
    if request.method == 'GET':
        ...
        data = pd.read_csv('Preprocessed_data/tweet_data_preprocessed.csv', usecols=range(13), index_col=False,
                           low_memory=False)
        return (pd.DataFrame.to_json(data, orient='index'))
...
def line2():
    if request.method == 'GET':
        res = []
        data = pd.read_csv('Prediction_data/simpsons_episodes.csv', usecols=range(19), index_col=False,
                           low_memory=False)
...
def bar2():
    if request.method == 'GET':
        ...
        neg = []
        for x in fileNames:
            data = pd.read_csv('Prediction_data/tweet_' + x + '_predict.csv', usecols=range(15), index_col=False,
                               low_memory=False)
...
def sentiment():
    if request.method == 'GET':
        ...
        data = pd.read_csv('Prediction_data/tweet_data_predict.csv', usecols=range(15), index_col=False,
                           low_memory=False)
        return (pd.DataFrame.to_json(data, orient='index'))
