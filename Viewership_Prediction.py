...
def main(simpsons_file):
    ...
    viewer_data = pd.read_csv(simpsons_file, dtype={'Unique_Users': float, 'US_Viewers_In_Millions': float},
                              usecols=range(19), index_col=False, low_memory=False)
    viewer_data.dropna(inplace=True)
    print('Episode Data File Read Successful')

    x = viewer_data.loc[:,
        ['Views', 'IMDB_Rating', 'IMDB_Votes', 'Retweets', 'Favorites', 'Vader_Score', 'Sentiment_Score',
